#!/usr/bin/env bash
# 역할: 최신 agent image로 Czardas PostgreSQL schema Job을 1회 실행합니다.
set -euo pipefail
...
JOB_NAME="${CZARDAS_ASSET_MIGRATIONS_JOB_NAME:-czardas-asset-migrations}"
MANIFEST="${CZARDAS_ASSET_MIGRATIONS_MANIFEST:-infra/k8s/base/job-czardas-asset-migrations.yaml}"
TIMEOUT="${CZARDAS_ASSET_MIGRATIONS_TIMEOUT:-600s}"

tmp_dir="$(mktemp -d)"
trap 'rm -rf "${tmp_dir}"' EXIT
rendered="${tmp_dir}/job-czardas-asset-migrations.yaml"
sed \
  -e "s#^  name: czardas-asset-migrations\$#  name: ${JOB_NAME}#" \
