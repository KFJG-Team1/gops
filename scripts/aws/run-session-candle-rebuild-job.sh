...
if [[ "${APPLY}" == "true" ]]; then
  kubectl -n "${NAMESPACE}" exec clickhouse-0 -- sh -c '
    clickhouse-client \
      ...
      --query "ALTER TABLE market_data.chart_candles ADD COLUMN IF NOT EXISTS bucket_policy LowCardinality(String) DEFAULT '\''clock_aligned'\'' AFTER price_adjustment"
    clickhouse-client \
      --user "$CLICKHOUSE_USER" \
      --password "$CLICKHOUSE_PASSWORD" \
      --query "ALTER TABLE market_data.chart_candles ADD COLUMN IF NOT EXISTS bucket_policy_key LowCardinality(String) AFTER bucket_policy"
  '
fi
