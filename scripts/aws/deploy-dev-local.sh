...
RUN_CZARDAS_ASSET_MIGRATIONS="${RUN_CZARDAS_ASSET_MIGRATIONS:-false}"
...
Optional environment variables:
  ...
  RUN_CZARDAS_ASSET_MIGRATIONS=true     Run Czardas asset migrations; requires agent-orchestrator selected.
...
validate_optional_tasks() {
  ...
  if is_true "${RUN_CZARDAS_ASSET_MIGRATIONS}" && ! service_selected "agent-orchestrator"; then
    printf 'RUN_CZARDAS_ASSET_MIGRATIONS=true requires agent-orchestrator to be selected.\n' >&2
    exit 1
  fi
...
run_migrations_if_requested() {
  ...
  if is_true "${RUN_CZARDAS_ASSET_MIGRATIONS}"; then
    if is_true "${DRY_RUN}"; then
      printf 'DRY_RUN=true: skipping Czardas asset migrations job.\n'
    else
      (
        cd "${WORKTREE_DIR}"
        # shellcheck source=scripts/aws/lib-gops-images.sh
