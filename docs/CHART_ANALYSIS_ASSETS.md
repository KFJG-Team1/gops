...
- 대응 시나리오: 형성 중 패턴의 정확한 완료 봉 확인 가격과, 확인된 패턴의
  진입 후보·초기/현재 무효화·T1/T2 분할 목표를 `tradePlan`으로 제공
...
`tradePlan`은 주문이 아니라 차트 표시용 시나리오다. v1/v2 응답은 계속 읽고 새 빌드는
`pattern-trade-timing-v3`를 생성한다. `confirmationConditions`는 방향·상/하단 경계·
경계 가격·확인 가격·ATR buffer·완료 봉 비교 규칙을 제공한다. 방향성 `forming` 패턴은
조건 하나, 방향이 정해지지 않은 대칭 삼각형은 상·하단 조건 두 개를 제공하며 진입·
무효화·목표·손익비는 비워 둔다. 돌파 기준은 패턴 경계에서 `0.25 ATR` 바깥의 완료 봉
종가다. 돌파 봉 거래량이 직전 20봉 중앙값의 `1.5배` 이상이거나 다음 완료 봉이 경계
밖을 유지해야 확정하며, 그 전에는 `confirmation_pending`이다.

확정 뒤 최대 5개 완료 봉에서 경계-트리거 구간 리테스트와 트리거 위 종가 마감을 찾는다.
유효 리테스트가 있으면 그 종가를, 없으면 확정 종가를 진입 후보로 사용한다. 초기 손절은
구조적 무효화와 돌파 경계 `1 ATR` 또는 리테스트 저가 `0.25 ATR` 기준 중 더 가까운 유효
가격이다. T1은 1R과 가장 가까운 활성 반대 레벨 중 가까운 가격이며 50% 대응을 제안한다.
T1이 0.75R보다 가깝거나, 측정 목표 T2가 2R 미만이거나 T1 이하이면 신규 진입을 보류한다.
T1 도달 뒤 현재 보호 가격은 진입가로 올린다. 같은 완료 봉에서 목표와 손절이 모두 닿으면
손절을 먼저 판정한다. 기본 운영 모드는 long-only라 하락 확인은 공매도 진입이 아니라
`매도·청산 후보`다.
...
flowchart LR
  ...
  API --> UI["Geometry + Scenario layers + SMA60/120"]
```
...
확인 신호는 실제 완료 봉에 `flagMarker`로 표시한다. 신규 포지션 후보의 진입·손절·T1·T2는
프런트가 `[entry, stop, target1, target2]` 순서의 `tradePlanBox`로 만들며, v1/v2 자산은
기존 `riskRewardBox`로 표시한다. 미래 봉 timestamp를 만들지 않고 화면에서만 미래
logical index로 투영한다. 교차 마커와 이 동적 도형은 PostgreSQL geometry drawing 예산
8개에 포함하지 않으며 주문 API를 호출하지 않는다. 차트는 `Geometry`와 `시나리오`
레이어를 독립적으로 제어한다. 시나리오 레이어는 확인 구간/선, 돌파·확인·리테스트 마커,
거래 계획 박스, `형성 → 돌파 확인 → 리테스트 → 분할 대응` 레일과 대응·무효화 카드를
함께 제어한다. 시스템 가격 라벨은 우측 사다리에 충돌 없이 정렬하고 실제 가격에는
leader line을 연결한다.
