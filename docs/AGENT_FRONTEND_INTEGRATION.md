...
`tradePlanBox`는 `[entry, stop, target1, target2]` 네 anchor를 사용하며 stop/T1/T2의
time을 하나의 미래 logical index로 정규화한다. 수동 도구는 4회 클릭, 선택·anchor drag·
삭제·preview·undo/redo를 지원하고 시스템 시나리오 인스턴스만 잠근다.
...
SMA60·SMA120과 최근 교차 상태를 표시한다. Geometry 토글은 레벨·패턴·SMA 교차를,
시나리오 토글은 확인 가격 구간/선·돌파/확인/리테스트 마커·거래 계획 박스·단계 레일과 해설 패널의
대응/무효화 카드를 제어한다. 두 상태는 chart document 단위 세션 상태로 유지한다.
패턴 선은 실선, forming은 낮은 불투명도로 표현한다. 새 자산은
...
v3 `tradePlan.confirmationConditions`는 백엔드가 완료 봉과 `0.25 ATR`로 계산한 정확한
확인 가격이다. `forming/watch`는 주황색 확인선과 관찰 마커만 만들고 진입·무효화·목표를
표시하지 않는다. 가격만 돌파하면 빈 마커와 거래량/다음 봉 확인 대기를 표시한다.
거래량 `1.5×` 또는 다음 완료 봉 유지 확인 뒤 최대 5봉 동안 리테스트를 관찰한다.
방향이 없는 대칭 패턴은 상·하단 확인선 두 개를 허용한다.
`tradePlan.action`이 `buy_candidate`이면 확인/리테스트 완료 봉의 `flagMarker`와
`[entry, stop, target1, target2]` 순서의 `tradePlanBox`를 함께 적용한다. v1/v2 자산은
기존 `riskRewardBox` 표시를 유지한다. long-only 기본값의
`sell_candidate`는 청산 의미의 `flagMarker`만 적용하며 공매도 박스를 만들지 않는다.
손익비가 기준 미만인 `no_trade`는 확인 마커와 진입 보류 사유만 표시한다.
박스의 Entry는 실제 확인 또는 리테스트 봉 timestamp를 사용하고 Stop/T1/T2의 미래 끝점은 자산에
저장하지 않는 logical index 투영만 사용해 가짜 candle timestamp를 만들지 않는다.
시스템 시나리오 박스는 C/E/S/T1/T2와 가격·비중·R을 우측 가격 사다리에 표시하고
leader line으로 실제 가격을 연결한다. 패턴 이름은 상단 단일 badge로 표시한다. 해당 가격은
차트 자동 범위에 포함한다. 수동 작도와 숨긴 시나리오는 자동 범위에 포함하지 않는다.
일·주봉 신호는 canonical candle key로 일치시킨다. 이 표시는 교육용 시나리오이며
주문 route를 호출하지 않는다.
