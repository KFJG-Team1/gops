...
def get_czardas_assets(
    ...
    try:
        ...
        assets = symbol_entries(normalized, records, czardas_identity_reader().current_identity)
...
def build_czardas_asset(
    ...
    return {
        ...
        "status": "queued",
        "status_url": f"/api/charts/czardas-assets/build/{envelope.job_id}",
    }
...
def get_czardas_build_status(
    ...
    state = czardas_asset_progress_store().get(job_id)
    if state is None:
...
def cancel_czardas_build(
    ...
    state = czardas_asset_progress_store().request_cancel(job_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Czardas build job not found.")
    return state
