...
class MarketDataHardeningContractTest(unittest.TestCase):
    ...
    def test_feed_profiles_and_market_session_contract(self):
        ...
        monday_pre_windows = visible_extended_session_windows(datetime(2026, 6, 29, 8, 30, tzinfo=timezone.utc))
        self.assertEqual([session for session, _, _ in monday_pre_windows], ["overnight", "pre"])
        pre_windows = visible_extended_session_windows(datetime(2026, 6, 30, 8, 30, tzinfo=timezone.utc))
        self.assertEqual([session for session, _, _ in pre_windows], ["after", "overnight", "pre"])
        self.assertEqual(pre_windows[0][1], datetime(2026, 6, 29, 20, 0, tzinfo=timezone.utc))
        self.assertEqual(pre_windows[0][2], datetime(2026, 6, 30, 0, 0, tzinfo=timezone.utc))
        self.assertTrue(all(left[2] == right[1] for left, right in zip(pre_windows, pre_windows[1:])))
