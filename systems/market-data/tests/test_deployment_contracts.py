...
class DeploymentContractsTest(unittest.TestCase):
    def test_fresh_candle_ddl_preserves_canonical_variants_without_runtime_key_rewrite(self):
        for path in (
            "infra/clickhouse/initdb/01-market-data.sql",
            "infra/k8s/base/platform/clickhouse-initdb/01-market-data.sql",
        ):
            ddl = (REPO_ROOT / path).read_text(encoding="utf-8")
            chart_table = ddl[ddl.index("CREATE TABLE IF NOT EXISTS market_data.chart_candles") :]
            chart_table = chart_table[: chart_table.index("CREATE TABLE IF NOT EXISTS", 1)]
            self.assertIn("canonical_version,\n    price_adjustment", chart_table)
            self.assertNotIn("MODIFY ORDER BY", ddl)

        rebuild = (REPO_ROOT / "scripts/aws/run-session-candle-rebuild-job.sh").read_text(encoding="utf-8")
        self.assertNotIn("MODIFY ORDER BY", rebuild)

    def test_czardas_runtime_is_postgres_alpaca_only(self):
        base = load_yaml("infra/k8s/base/app/configmap.yaml")["data"]
        self.assertEqual(base["ALPACA_CREDENTIAL_SOURCE"], "local-env")
        self.assertFalse(any(name.startswith("CHART_ASSET_") for name in base))

        compose_services = load_yaml("docker-compose.yml")["services"]
        self.assertNotIn("chart-asset-builder", compose_services)
        compose_builder = compose_services["czardas-asset-builder"]
        compose_env = compose_builder["environment"]
        self.assertEqual(compose_env["ALPACA_CREDENTIAL_SOURCE"], "local-env")
        self.assertIn("APCA_API_KEY_ID", compose_env)
        self.assertIn("APCA_API_SECRET_KEY", compose_env)
        for name in ("REDIS_URL", "KAFKA_BOOTSTRAP_SERVERS", "S3_BUCKET", "OPENAI_API_KEY"):
            self.assertNotIn(name, compose_env)
        self.assertFalse(any(name.startswith("CHART_ASSET_") for name in compose_env))
        ...
        self.assertEqual(data["ALPACA_CREDENTIAL_SOURCE"], "aws-secrets-manager")
        self.assertEqual(data["ALPACA_SECRET_NAME"], "dev/alpaca")
        self.assertFalse(any(name.startswith("CHART_ASSET_") for name in data))

    def test_czardas_postgres_migration_is_explicit_and_builder_has_db_secret(self):
        ...
        self.assertIn("czardas-asset-migrations/main.py", " ".join(container["command"]))
        self.assertEqual(
        ...
        runner = (REPO_ROOT / "scripts/aws/run-czardas-asset-migrations-job.sh").read_text(encoding="utf-8")
        ...
        compose_migration = compose["services"]["czardas-asset-migrations"]
        self.assertNotIn("profiles", compose_migration)
        self.assertNotIn("CHART_ASSET_STORAGE_MAINTENANCE", compose_migration["environment"])
        self.assertEqual(
            compose["services"]["czardas-asset-builder"]["depends_on"]["czardas-asset-migrations"]["condition"],
            "service_completed_successfully",
        )

    def test_legacy_geometry_cleanup_is_explicit_and_preview_first(self):
        cleanup = REPO_ROOT / "scripts/aws/retire-legacy-chart-geometry.sh"
        body = cleanup.read_text(encoding="utf-8")
        self.assertIn("deployment/chart-asset-builder", body)
        self.assertIn("cronjob/chart-geometry-build", body)
        self.assertIn("--ignore-not-found", body)
        self.assertNotIn("geometry_", body)
        self.assertNotIn("chart_analysis_assets", body)
        self.assertNotIn("DROP ", body.upper())

        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            calls = temp / "kubectl.calls"
...
""",
            ...
            env = {
                **os.environ,
                ...
                "K8S_NAMESPACE": "czardas-test",
            }
            subprocess.run([str(cleanup)], cwd=REPO_ROOT, env=env, check=True, capture_output=True, text=True)
            preview_calls = calls.read_text(encoding="utf-8")
            self.assertIn("get deployment/chart-asset-builder cronjob/chart-geometry-build", preview_calls)
            self.assertNotIn("delete", preview_calls)
            ...
            apply_calls = calls.read_text(encoding="utf-8")
            self.assertIn("delete deployment/chart-asset-builder cronjob/chart-geometry-build", apply_calls)
            self.assertIn("--ignore-not-found", apply_calls)

        for path in ("README.md", "docs/ONBOARDING_LOCAL_DOCKER.md"):
            startup = (REPO_ROOT / path).read_text(encoding="utf-8")
            self.assertIn(
                "docker compose --env-file .env up -d --build --remove-orphans",
                startup,
            )

    def test_czardas_build_is_manual_and_geometry_schedule_is_absent(self):
        scheduled = load_yaml("infra/k8s/overlays/aws/scheduled/kustomization.yaml")["resources"]
        self.assertFalse(any("geometry" in resource or "czardas" in resource for resource in scheduled))
        self.assertFalse((REPO_ROOT / "infra/k8s/overlays/aws/scheduled/cronjob-chart-geometry-build.yaml").exists())
        self.assertFalse((REPO_ROOT / "scripts/aws/run-chart-geometry-build-job.sh").exists())
    ...
    def test_agent_shared_changes_rebuild_agent_and_backend_images(self):
        ...
        self.assertIn("czardas-asset-builder", agent_case)
        self.assertNotIn("chart-asset-builder", agent_case)
...
""",
            ...
            completed = subprocess.run(
                ...
                env={
                    **base_env,
                    "CZARDAS_ASSET_MIGRATIONS_JOB_NAME": "czardas-assets-verify",
                    "K8S_NAMESPACE": "czardas-assets-test",
                },
                check=True,
                capture_output=True,
                text=True,
            ...
            self.assertEqual(applied["metadata"]["name"], "czardas-assets-verify")
            self.assertEqual(applied["metadata"]["namespace"], "czardas-assets-test")
