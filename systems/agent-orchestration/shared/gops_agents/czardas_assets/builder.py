        ...
        try:
            ...
            for snapshot_attempt in range(2):
                repair = self.repair.ensure_exact(
                    symbol,
                    interval,
                    request_id=f"{envelope.job_id}-snapshot-{snapshot_attempt + 1}",
                ...
                if not repair.window.ready:
                    item = _item(
                        ...
                        error=f"{len(repair.window.rows)} of 240 canonical completed candles available",
                    )
                    self.progress.record_item(envelope.job_id, item)
                    return item
                ...
                unchanged = (
                    not envelope.force
                    and existing
                    and existing.get("inputDigest") == repair.window.input_digest
                ...
                result = None if unchanged else analyze_czardas(repair.window.rows)
            ...
            if save_state is None:
                raise RuntimeError("Czardas snapshot loop ended without a storage decision")
