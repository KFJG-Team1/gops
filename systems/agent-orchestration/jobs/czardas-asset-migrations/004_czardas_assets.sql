...
CREATE TABLE IF NOT EXISTS chart_assets.czardas_latest (
    ...
    field_bytes INTEGER NOT NULL CHECK (field_bytes BETWEEN 0 AND 81920),
    payload_bytes INTEGER NOT NULL CHECK (payload_bytes BETWEEN 0 AND 98304),
...
ALTER TABLE chart_assets.czardas_latest
    ADD CONSTRAINT czardas_latest_field_bytes_check
    CHECK (field_bytes BETWEEN 0 AND 81920);
ALTER TABLE chart_assets.czardas_latest
    DROP CONSTRAINT IF EXISTS czardas_latest_payload_bytes_check;
ALTER TABLE chart_assets.czardas_latest
    ADD CONSTRAINT czardas_latest_payload_bytes_check
    CHECK (payload_bytes BETWEEN 0 AND 98304);
