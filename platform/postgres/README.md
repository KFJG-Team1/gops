...
the pack. The v4 all-candle Field and pack are limited to 80/96 KiB. Czardas IDs use `cza-` and
...
`004_czardas_assets.sql` followed by the additive
`005_czardas_v3_identity_and_ops.sql`. The historical `v3` filename is immutable;
its identity and queue columns are also used by v4 packs.
...
successfully. Runtime never creates or alters these tables itself.
