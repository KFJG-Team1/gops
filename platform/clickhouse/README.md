...
While a pre, after, or overnight session is active, readers also query the
bounded uninterrupted extended-session chain through the current session
(`after -> overnight -> pre`) and expose a session-anchored
`us_equity_extended_session` aggregate. This aggregate is a
