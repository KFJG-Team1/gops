...
export type TimeAxisBoundaryKind = "day" | "week" | "month" | "year";
...
export function buildTimeAxisLayout(candles: CandleData[], timeframe: string, plotWidth: number): TimeAxisLayout {
  if (candles.length === 0) {
    return { labels: [], boundaries: [] };
  }

  const interval = normalizeChartInterval(timeframe) ?? "1m";
  const slotWidth = Math.max(1, plotWidth) / Math.max(1, candles.length);
...
function boundaryKind(previous: CalendarParts, current: CalendarParts, interval: ChartInterval): TimeAxisBoundaryKind | null {
  ...
  if (interval === "1D" && weekStartKey(current) !== weekStartKey(previous)) {
    return "week";
  }
  if (interval === "1W" && calendarMonthKey(current) !== calendarMonthKey(previous)) {
    return "month";
  }
...
function weekStartKey(parts: CalendarParts): string {
