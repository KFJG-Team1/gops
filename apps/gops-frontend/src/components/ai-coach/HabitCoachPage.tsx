...
function explainPattern(pattern: HabitPattern, report?: HabitReport): string {
  const sampleSize = report?.sampleSize ?? 0;
  const total = report?.totalTradeCount ?? sampleSize;
  const analyzed = report?.analyzedTradeCount ?? sampleSize;
  const basis = total !== analyzed
    ? `전체 ${formatNumber(total)}건 중 분석 가능한 ${formatNumber(analyzed)}건 기준으로`
    : `${formatNumber(analyzed)}건 기준으로`;

  if (pattern.id.startsWith("missed-")) {
    return sampleSize
      ? `${basis}, ${formatNumber(pattern.occurrenceCount)}건에서 ${pattern.title.replace(" 확인 누락", "")}을 놓쳤습니다. 다음 거래에서 먼저 줄여야 하는 항목입니다.`
      : pattern.description;
  }

  if (pattern.id === "decision-record-gap") {
    return sampleSize
      ? `${basis}, ${formatNumber(pattern.occurrenceCount)}건은 주문 전 판단 기록이 없어 무엇을 확인했는지 검증하기 어렵습니다.`
      : pattern.description;
  }
...
function tendencyTone(percent?: number | null): TendencyKeyword["tone"] {
