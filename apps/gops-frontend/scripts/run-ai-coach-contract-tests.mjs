...
assert.match(shell, /const PAGES = \["당일 거래 회고", "효과·보완 조건", "장기 습관", "실행·알람 관리"\]/);
assert.match(shell, /if \(page === 1\) return plan \? <ImprovementCoachPage/);
assert.match(shell, /if \(page === 2\) return resolved\?\.page2 \? <HabitCoachPage/);
assert.match(shellStyles, /\.shell\[data-page="2"\] > main/);
assert.doesNotMatch(shellStyles, /\.shell\[data-page="3"\]/);
