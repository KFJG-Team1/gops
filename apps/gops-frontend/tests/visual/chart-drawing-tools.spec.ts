...
test("drawing color palette applies global theme tokens to stroke, text, and supported fills", async ({ page }) => {
  ...
  const lineWidth = lineWidthGroup.getByRole("slider", { name: "선 두께 1에서 5" });
  await expect(lineWidth).toHaveValue("1");
  await expect(lineWidth).toHaveAttribute("min", "1");
  await expect(lineWidth).toHaveAttribute("max", "5");
  await expect(lineWidth).toHaveAttribute("step", "0.5");
  await lineWidth.fill("3.5");
  await expect(colorMenu).toBeVisible();
  await expect(lineWidth).toHaveValue("3.5");
  await expect(lineWidthGroup.locator("output")).toHaveText("3.5");
