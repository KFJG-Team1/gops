...
assert.equal(view.scenario?.status, "조건부 매수 시나리오");
assert.equal(view.scenario?.confirmation, "매수 검토 100.00 · 패턴 상단");
assert.deepEqual(view.keyPrices.map((item) => item.label), ["매수 검토", "수익 실현 검토", "손실 제한 검토"]);
