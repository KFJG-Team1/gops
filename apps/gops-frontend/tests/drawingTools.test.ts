...
const polylineDocument = createChartDocument("polyline-document", "AAPL", "1m");
const polylineAnchors = [drawingAnchor(0, 101), drawingAnchor(2, 106), drawingAnchor(5, 103)];
const polylineAdd = executeChartCommand(polylineDocument, makeChartCommand(
  "chart.drawing.add", "user", chartTarget(polylineDocument.id), {
    drawingType: "polyline",
    anchors: polylineAnchors,
    style: { colorToken: "drawing", lineWidth: 2, extension: "none" }
  }
));
assert.equal(polylineAdd.ok, true);
if (!polylineAdd.ok) assert.fail(polylineAdd.message);
assert.equal(polylineAdd.document.drawings[0]?.type, "polyline");
assert.deepEqual(
  polylineAdd.document.drawings[0]?.anchors.map((anchor) => [anchor.timestamp, anchor.price]),
  polylineAnchors.map((anchor) => [anchor.timestamp, anchor.price])
);
const restoredPolyline = restoreChartDocumentSnapshot(
  polylineAdd.document,
  JSON.parse(JSON.stringify(snapshotChartDocument(polylineAdd.document))) as ReturnType<typeof snapshotChartDocument>
);
assert.equal(restoredPolyline.drawings[0]?.type, "polyline");
assert.equal(restoredPolyline.drawings[0]?.anchors.length, 3);
const shortPolyline = executeChartCommand(polylineDocument, makeChartCommand(
  "chart.drawing.add", "user", chartTarget(polylineDocument.id), {
    drawingType: "polyline", anchors: polylineAnchors.slice(0, 2)
  }
));
assert.equal(shortPolyline.ok, false);
